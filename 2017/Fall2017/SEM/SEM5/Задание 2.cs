...
namespace Задание_2
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            long[] firstArray = new long[100];
            long[] thurdArray = new long[100];
            long mainNum = 0;
            int r = 0;
            while (k > 0)
            {
                firstArray[r] = k % 10;// переводим число из степени а, в 10-ую
                mainNum += (firstArray[r] * (int)Math.Pow(a, r));
                k /= 10;
                r++;
            }
            ...
            while (mainNum > 0)
            {
                secondArray[i] = (mainNum % b);//находим b-ую запись 10-ого числа
                mainNum /= b;
                i++;
                countNum++;
            }
...
>>>>>>> 0842e9e428d4a72eb850e02492cbf36fcd0dc148
